...
int main() {
    ...
    unsigned long long (*array)[4] = malloc(size * sizeof(unsigned long long[6]));

    for (long long i = 0; i < size; i++){
        array[i][0] = arr[i][2] * 10000 + arr[i][1] * 100 + arr[i][0] ;
        array[i][1] = arr[i][3];
        array[i][2] = arr[i][4];
        array[i][3] = arr[i][5];
    }
    ...
    qsort(array, size, sizeof(unsigned long long[4]), compare_by_first);
