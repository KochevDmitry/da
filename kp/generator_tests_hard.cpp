...
// Генерация случайного выпуклого многоугольника в пределах заданной области
std::vector<Point> generate_convex_polygon(int num_points, int min_x, int max_x, int min_y, int max_y) {
    std::set<Point> points_set;

    // Создание множества случайных уникальных точек в пределах заданной области
...
void generate_test(int total_points, int num_check_points, int min_coord, int max_coord, const std::string& filename) {
    ...
    while (remaining_points >= 3) { // Мы уверены, что остаётся хотя бы 3 точки для одного многоугольника
        ...
        // Генерация многоугольника, где каждый следующий многоугольник будет выше
        auto new_polygon = generate_convex_polygon(num_points, offset_x, offset_x + step, offset_y, offset_y + step);
        polygons.push_back(new_polygon);

        // Смещаем координаты для следующего многоугольника
        offset_y += step;
...
int main() {
    ...
    int num_check_points = 10000; // Количество точек на проверку
    int min_coord = 0, max_coord = 100000; // Диапазон координат
    std::string filename = "test_hard.txt";  // Имя выходного файла
